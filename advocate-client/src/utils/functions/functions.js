...
export const mapStudents = (students) => students.map(stu => ({id: stu.id, name: stu.name, age: stu.age, grade: stu.grade}))

export const objectIsEmpty = (object) => {
  const [keys, values] = Object.entries(object);
  return !Boolean(keys?.length) && !Boolean(values?.length);
}
...
export const removeEntryFromObject = (keyToRemove, obj) => {
  let newObj = {};
  Object.keys(obj).forEach(key => {
    if((keyToRemove.length ? !keyToRemove.includes(key) : key !== keyToRemove))
