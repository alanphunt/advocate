...
note: the object sent back to the parent will not be the full data model, but only the formatted version passed into this component
    Props:
        ...
        columnSize: object: optional- {rowIndex: "class"}
    State:
...
const Table = ({
 ...
 columnSize
  ...
  const adjustedColumnSize = (index) => {
    return columnSize ? (Object.keys(columnSize).includes(index.toString()) ? columnSize[index] : "") : "";
  }
        ...
        headers ? (
          <TableHeaderRow>
            { headers.map((header, index) =>
              <TableHeaderCell key={`tableHeaderCell-${index}`} classes={adjustedColumnSize(index)}>
                {
                  hideSearchAndSort ? (
                    header
                  ) : (
          ...
          children ? children :
            sortedData.map((rowObject, rowIndex) =>
              <TableRow key={`tableRow-${rowIndex}`} isSelected={(selectedRowId || -1) === rowObject.id} selectedCallback={selectedCallback ? () => selectedCallback(rowObject, rowIndex) : null}>
                {
                  tableKeys.filter(key => key !== "id").map((key, ind) => <TableCell key={`tableCell-${ind}-${rowIndex}`} classes={adjustedColumnSize(ind)}>{rowObject[key]}</TableCell>)
                }
              </TableRow>
            )
        }
